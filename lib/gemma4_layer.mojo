...
def _mm_dev(
    ...
    if global_scale != 0.0 and w4a4.on and not is_sm100:
        ...
        # blob directly, no bf16 materialization. (GLM/swarm 2026-06-25; UNIT_A_Q4_OCCUPANCY.md)
...
def prepare_qkv_dev(
    ...
    # 1. Input rmsnorm: d_x → d_normed
    gpu_rmsnorm_mojo(ctx, d_x, d_normed, d_in_norm_w, d)
    ...
    if act_precision() == 8 and d_q4_scratch != 0 and g_q == 0.0:
        ...
        if is_full:
            cuda_memcpy(d_v_new, d_k_new, l_kvd * 4, 3)  # V=K device-to-device
        else:
            gpu_matmul_q4_dp4a_gemv_dev[4](ctx, d_v_new, d_scratch_in_bf16, d_vw, d, l_kvd)
    elif (
        w4a4.on and not _is_sm100_dev(ctx)
        and g_q != 0.0 and g_k != 0.0 and (is_full or g_v != 0.0)
        and ag_q == ag_k and (is_full or ag_q == ag_v)
    ):
        ...
        _mm_dev(ctx, handle, d_q, d_normed, d_qw, d_scratch_in_bf16, d, l_qd, d_q4_scratch, g_q, ag_q, w4a4)
        _mm_dev(ctx, handle, d_k_new, d_normed, d_kw, d_scratch_in_bf16, d, l_kvd, d_q4_scratch, g_k, ag_k, w4a4)
        if is_full:
            cuda_memcpy(d_v_new, d_k_new, l_kvd * 4, 3)  # cudaMemcpyDeviceToDevice
        else:
            _mm_dev(ctx, handle, d_v_new, d_normed, d_vw, d_scratch_in_bf16, d, l_kvd, d_q4_scratch, g_v, ag_v, w4a4)

    # 3. V-norm: per-head RMSNorm without learnable weight
    gpu_rmsnorm_no_weight_mojo(ctx, d_v_new, l_nkv, l_hd)

    # 4. QK norm
    gpu_qk_norm_mojo(ctx, d_q, d_q_norm_w, l_nh, l_hd)
    gpu_qk_norm_mojo(ctx, d_k_new, d_k_norm_w, l_nkv, l_hd)

    # 5. RoPE
    if is_full:
        var rdim = l_hd // 4
        gpu_rope_mojo(ctx, d_q, pos, l_nh, l_hd, 1000000.0, rdim)
        gpu_rope_mojo(ctx, d_k_new, pos, l_nkv, l_hd, 1000000.0, rdim)
    else:
        gpu_rope_mojo(ctx, d_q, pos, l_nh, l_hd, 10000.0, l_hd)
        gpu_rope_mojo(ctx, d_k_new, pos, l_nkv, l_hd, 10000.0, l_hd)
...
def prepare_qkv_batched(
    ...
    # 1. Input rmsnorm (per row)
    gpu_rmsnorm_batched_mojo(ctx, d_x, d_normed, d_in_norm_w, d, S)

    # 2. Q,K,V projections (GEMM, M=S) — Q4/W4A4-aware via _mm_dev_batched
    _mm_dev_batched(ctx, handle, d_q, d_normed, d_qw, d_scratch_in_bf16, S, d, l_qd, d_q4_scratch, g_q, ag_q, w4a4)
    _mm_dev_batched(ctx, handle, d_k_new, d_normed, d_kw, d_scratch_in_bf16, S, d, l_kvd, d_q4_scratch, g_k, ag_k, w4a4)
    if is_full:
        cuda_memcpy(d_v_new, d_k_new, S * l_kvd * 4, 3)  # V = K, device-to-device
    else:
        _mm_dev_batched(ctx, handle, d_v_new, d_normed, d_vw, d_scratch_in_bf16, S, d, l_kvd, d_q4_scratch, g_v, ag_v, w4a4)

    # 3. V-norm: per (token,head) RMSNorm, no weight — reuse per-head kernel over S*n_kv rows
    gpu_rmsnorm_no_weight_mojo(ctx, d_v_new, S * l_nkv, l_hd)

    # 4. QK norm: per (token,head), shared weight — reuse per-head kernel over S*heads rows
    gpu_qk_norm_mojo(ctx, d_q, d_q_norm_w, S * l_nh, l_hd)
    gpu_qk_norm_mojo(ctx, d_k_new, d_k_norm_w, S * l_nkv, l_hd)

    # 5. RoPE (per-token absolute position)
    if is_full:
        var rdim = l_hd // 4
        gpu_rope_batched_mojo(ctx, d_q, base_pos, S, l_nh, l_hd, 1000000.0, rdim)
        gpu_rope_batched_mojo(ctx, d_k_new, base_pos, S, l_nkv, l_hd, 1000000.0, rdim)
    else:
        gpu_rope_batched_mojo(ctx, d_q, base_pos, S, l_nh, l_hd, 10000.0, l_hd)
        gpu_rope_batched_mojo(ctx, d_k_new, base_pos, S, l_nkv, l_hd, 10000.0, l_hd)
...
def apply_output_and_mlp_batched(
    ...
    # 8-9. O projection → post-attn norm → residual
    _mm_dev_batched(ctx, handle, d_o_out, d_attn_out, d_ow, d_scratch_in_bf16, S, l_qd, d, d_q4_scratch, g_o, ag_o, w4a4)
    if prof_stages:
    ...
    gpu_rmsnorm_inplace_batched_mojo(ctx, d_o_out, d_post_attn_norm_w, d, S)
    ...
    gpu_rmsnorm_batched_mojo(ctx, d_x, d_pn, d_pre_ff_norm_w, d, S)
    ...
    if prof_stages:
        ctx.synchronize(); _pt0 = Int(perf_counter_ns())
    _mm_dev_batched(ctx, handle, d_gate, d_pn, d_gw, d_scratch_in_bf16, S, d, ff, d_q4_scratch, g_g, ag_g, w4a4)
    if prof_stages:
        ctx.synchronize(); t_gate = Int(perf_counter_ns()) - _pt0; _pt0 = Int(perf_counter_ns())
    _mm_dev_batched(ctx, handle, d_up, d_pn, d_uw, d_scratch_in_bf16, S, d, ff, d_q4_scratch, g_u, ag_u, w4a4)
    if prof_stages:
    ...
    _mm_dev_batched(ctx, handle, d_down, d_mlp_v, d_dw, d_scratch_in_bf16, S, ff, d, d_q4_scratch, g_d, ag_d, w4a4)
    if prof_stages:
    ...
    gpu_rmsnorm_inplace_batched_mojo(ctx, d_down, d_post_ff_norm_w, d, S)
...
def apply_output_and_mlp_dev(
    ...
    # 8-9. O projection → post-attn norm → residual
    _mm_dev(ctx, handle, d_o_out, d_attn_out, d_ow, d_scratch_in_bf16, l_qd, d, d_q4_scratch, g_o, ag_o, w4a4)
    if budget_prof: cuda_budget_mark(ctx, 3)
    if debug_out != 0 and debug_stage == 1:
        ctx.synchronize()
        cuda_memcpy(UInt64(debug_out), d_o_out, d * 4, 2)
    gpu_rmsnorm_inplace_mojo(ctx, d_o_out, d_post_attn_norm_w, d)
    ...
    # 10-12. Pre-FF norm → gate, up → GeGLU → down → post-FF norm → residual
    gpu_rmsnorm_mojo(ctx, d_x, d_pn, d_pre_ff_norm_w, d)
    ...
    if act_precision() == 8 and d_q4_scratch != 0 and g_g == 0.0:
        ...
        _mm_dev(ctx, handle, d_gate, d_pn, d_gw, d_scratch_in_bf16, d, ff, d_q4_scratch, g_g, ag_g, w4a4)
        if budget_prof: cuda_budget_mark(ctx, 5)
        _mm_dev(ctx, handle, d_up, d_pn, d_uw, d_scratch_in_bf16, d, ff, d_q4_scratch, g_u, ag_u, w4a4)
        if budget_prof: cuda_budget_mark(ctx, 6)
    gpu_gelu_mul_mojo(ctx, d_mlp_v, d_gate, d_up, ff)
    if budget_prof: cuda_budget_mark(ctx, 7)
    _mm_dev(ctx, handle, d_down, d_mlp_v, d_dw, d_scratch_in_bf16, ff, d, d_q4_scratch, g_d, ag_d, w4a4)
    if budget_prof: cuda_budget_mark(ctx, 8)
    if debug_out != 0 and debug_stage == 5:
        ctx.synchronize()
        cuda_memcpy(UInt64(debug_out), d_down, d * 4, 2)
    gpu_rmsnorm_inplace_mojo(ctx, d_down, d_post_ff_norm_w, d)
