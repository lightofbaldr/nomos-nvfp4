...
PIXI_ENV_DIR="$PWD/.pixi/envs/default"
CUDA_ROOT="${CUDA_HOME:-$PIXI_ENV_DIR}"
...
pixi run mojo build --emit shared-lib breeze_codec_ffi.mojo -o libnomos_codec-qwen3_tts.so \
  -Xlinker nomos_libc_shim.o -Xlinker nomos_token_cb_stub.o \
  -Xlinker "-L$CUDA_LIB" -Xlinker -rpath -Xlinker "$CUDA_LIB" \
